...
public class Reconstruct {
	...
	public void identifyParts(){
		...
		boolean match= true;
		for(int i=0; i<(ReconstructVector.PARTSPERNODE); i++  ){
			for(int j= 0; j< ReconstructVector.VECTORSIZE; j++ ){	
				if (ReconstructVector.list[failed_node][i][j] == 1){
					printReconstructVector(i);
					System.out.println();
					findMatchFromGeneratingNodes(i,j);
					/// Additional step to identify direct object match////
					System.out.println("Starting to test for direct match");
					if (!compareDirectEquality(i)){
						//////////////////////////////////////////////////////
						System.out.println("No Direct Match! Starting to compute XOR to find match.");
						match = startXORComparison(i);
					}
					if(match){
	...
	public boolean compareDirectEquality(int failed_part){
		int[] temp = new int[BasisVector.VECTORSIZE];
		boolean found = true;
		for(int i=0; i<mem_bit1.size(); i++){
			int node = mem_bit1.get(i)/10;
			int part = mem_bit1.get(i)%10;
			//printBasisVector(node, part);
			temp = returnBasisVector(node, part);
			for(int j=0; j<BasisVector.VECTORSIZE; j++){
				if(ReconstructVector.list[failed_node][failed_part][j] != temp[j]){
					found = false;
				}
			}
			if (found){
				System.out.println("!!!SUCCESS!!!");
				Integer [] store_temp = new Integer[1];
				store_temp[0] = mem_bit1.get(i);
				to_fetch.add(store_temp);
				return true;
			}
			found = true;
		}
	...
	public int identifyNextBitPosition(int failednode_part, int failednode_firstbit_position){
		for (int i=failednode_firstbit_position+1; i<BasisVector.VECTORSIZE; i++){
			if (ReconstructVector.list[failed_node][failednode_part][i] == 1){
				return i;
			}
		}
		return -1;
	...
	public boolean compareEqualityIntermediate(int part, Integer[] intermediateVector){
		boolean match = true;
		for (int i=0; i< BasisVector.VECTORSIZE; i++){
			if(ReconstructVector.list[failed_node][part][i] != intermediateVector[i] ){
				match = false;
			}
		}
	...
	public void printReconstructVector(int part){
		System.out.print("");
		for(int i =0; i<ReconstructVector.VECTORSIZE; i++){
			System.out.print(ReconstructVector.list[0][part][i]);
		}
	}

	public int[] returnBasisVector(int node, int part){
		int[] temp = new int[BasisVector.VECTORSIZE];
		for(int i =0; i<BasisVector.VECTORSIZE; i++){
			temp[i] = BasisVector.list[node][part][i];
		}
		return temp;
	}
	...
	public boolean compareEquality(int part){
		boolean match = true;
		for (int i =0; i<BasisVector.VECTORSIZE; i++){
			if (ReconstructVector.list[failed_node][part][i] != gen_vector.get(gen_vector.size()-1)[i]){
				match = false;
				break;
			}
